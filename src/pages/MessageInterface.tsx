...
const ChatInput = ({ onSendMessage }: { onSendMessage: (message: string) => void }) => {
  ...
  return (
    <div className="relative"> {/* Wrap everything in a relative container */}
    ...
    {/* Pop-Up Section */}
    {showOptions && (
      <div className="absolute bottom-16 left-0 w-full bg-white shadow-lg rounded-xl p-4 flex justify-between"> {/* Position the pop-up */}
...
const ChatScreen = () => {
  ...
  const handleSendMessage = (message: string) => {
    setMessages([...messages, { text: message, isSent: false }]); // Add the new message as sent
  };
        ...
        <div className="flex-1  flex flex-col overflow-y-auto p-4 bg-gray-50">
          {messages.length > 0 ? (
            messages.map((msg, index) => (
                <div
                key={index}
                className={`p-2 px-4 my-1 rounded-lg max-w-fit ${
                  msg.isSent
                    ? 'bg-blue-100 self-end text-right'  
                    : 'bg-gray-200 self-start text-left' 
                } flex justify-${msg.isSent ? 'start'  : 'end'}`} 
              >
                <span>{msg.text}</span> 
              </div>
            ))
